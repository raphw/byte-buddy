...
public class PluginEngineTargetDiscardingTest {
    ...
    public void testDiscarding() throws Exception {
        assertThat(Plugin.Engine.Target.Discarding.INSTANCE.write(), is((Plugin.Engine.Target.Sink) Plugin.Engine.Target.Discarding.INSTANCE));
        ...
        Plugin.Engine.Target.Discarding.INSTANCE.write().retain(eleement);
        verifyZeroInteractions(eleement);
        Plugin.Engine.Target.Discarding.INSTANCE.write().store(Collections.singletonMap(TypeDescription.OBJECT, new byte[]{1, 2, 3}));
    }
}
