...
public class MethodCallTypeTest extends AbstractInstrumentationTest {
    ...
    public void testFieldConstantPool() throws Exception {
        DynamicType.Loaded<Foo> loaded = instrument(Foo.class, MethodCall.invokeSuper().with(value));
    ...
    public void testFieldReference() throws Exception {
        DynamicType.Loaded<Foo> loaded = instrument(Foo.class, MethodCall.invokeSuper().withReference(value));
        assertThat(loaded.getLoadedAuxiliaryTypes().size(), is(0));
        assertThat(loaded.getLoaded().getDeclaredMethods().length, is(1));
        assertThat(loaded.getLoaded().getDeclaredMethod(FOO, Object.class), not(nullValue(Method.class)));
        assertThat(loaded.getLoaded().getDeclaredConstructors().length, is(1));
    ...
    public void testNonAssignable() throws Exception {
        instrument(Foo.class, MethodCall.invokeSuper().with(value).withAssigner(nonAssigner, false));
    }
