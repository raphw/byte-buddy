...
public class MethodCallProxy implements AuxiliaryClass {
    ...
    public MethodCallProxy(MethodDescription methodDescription) {
        Map<String, String> fieldTypes = new HashMap<String, String>(1 + methodDescription.getParameterTypes().length);
        int i = 0;
        ...
        for (Class<?> parameterType : methodDescription.getParameterTypes()) {
            fieldTypes.put(FIELD_NAME_PREFIX + i, Type.getDescriptor(parameterType));
            proxyConstructorDescriptor.append(Type.getDescriptor(parameterType));
            TypeSize argumentSize = TypeSize.of(parameterType);
            stackedSize = stackedSize.aggregate(argumentSize.toIncreasingSize());
            maximalSize = maximalSize.maximum(argumentSize);
        }
        ...
        proxiedTypeInternalName = methodDescription.getDeclaringClassInternalName();
    }
