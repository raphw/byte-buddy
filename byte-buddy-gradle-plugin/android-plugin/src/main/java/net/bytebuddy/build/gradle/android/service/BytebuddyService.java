...
abstract public class BytebuddyService implements BuildService<BytebuddyService.Params>, AutoCloseable {
    ...
    public BytebuddyService() {
        ...
        try {
            Set<File> classpath = parameters.getRuntimeClasspath().plus(parameters.getAndroidBootClasspath()).plus(parameters.getByteBuddyClasspath()).plus(parameters.getLocalClassesDirs()).getFiles();
            classFileLocator = getClassFileLocator(classpath);
            typePool = poolStrategy.typePool(classFileLocator);
            List<? extends Plugin.Factory> factories = createPluginFactories(parameters.getAndroidBootClasspath().getFiles(), parameters.getByteBuddyClasspath().getFiles());
