...
public final class BytebuddyManager {
    ...
    public synchronized static void initialize(FileCollection runtimeClasspath,
        ...
        try {
            classFileLocator = getClassFileLocator(runtimeClasspath.plus(androidBootClasspath).plus(byteBuddyClasspath));
    ...
    private static ClassFileLocator getClassFileLocator(FileCollection classpath) throws IOException {
        ArrayList<ClassFileLocator> classFileLocators = new ArrayList<>();

        for (File artifact : classpath.getFiles()) {
